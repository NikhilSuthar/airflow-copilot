...
class Summarization(object):
    ...
    def get_summary(message, summary = "") -> str:
        ...
        if len(message) > 0:
            model = init_chat_model(model=Summarization._env.summarization_model_name, 
                                    model_provider=Summarization._env.summarization_provider_name, 
                                    temperature=0, 
                                    api_key=Summarization._env.api_key)
    ...
    def summarize_message(state:  GraphState):
        MIN_MSG_TO_RETAIN = Summarization._env.min_msg_to_retain
        MIN_MSG_TO_SUMMARIZE = Summarization._env.min_msg_to_summarize
